...
import { enlaceWhatsApp, pesos, presentacion, precioPorContenido, type ProductoMayoreo } from '@/lib/mayoreo';
...
export function ConsultarProducto({ nombre = 'productos de mayoreo' }: { nombre?: string }) {
  ...
  return enlace ? <a className="b2b-consultar" href={enlace} target="_blank" rel="noopener noreferrer">Consultar por WhatsApp</a> : <p className="b2b-nota">Atención por WhatsApp próximamente</p>;
}
...
export default function CatalogoMayoreo({ productos }: { productos: ProductoMayoreo[] }) {
  return <div className="b2b-productos">{productos.map((p) => <article className="b2b-tarjeta" key={p.id}>
    <Link href={`/catalogo/${p.slug}`} aria-label={`Ver ${p.nombre}`}><FotoProducto producto={p} /></Link>
    <div className="b2b-tarjeta-cuerpo"><span className="b2b-etiqueta">{p.categoria}</span><h3><Link href={`/catalogo/${p.slug}`}>{p.nombre}</Link></h3>
      <p>{presentacion(p)}</p><strong className="b2b-precio">{p.precio_mayoreo ? pesos(p.precio_mayoreo) : 'Precio por confirmar'}</strong>
      {precioPorContenido(p) && <p>{precioPorContenido(p)}</p>}<p className="b2b-nota">Disponibilidad sujeta a confirmación</p>
      {p.precio_mayoreo && p.unidad && p.id !== p.slug ? <BotonAgregar producto={{ ...p, unidad: p.unidad, precio_mayoreo: p.precio_mayoreo }} /> : <span className="b2b-pendiente">Próximamente disponible para compra</span>}
