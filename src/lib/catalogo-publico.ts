...
const CAMPOS_MAYOREO = CAMPOS + ',slug,piezas_por_caja,bolsas_por_caja,peso_por_bolsa_g';
export const obtenerCatalogo = cache(async (): Promise<{ productos: ProductoMayoreo[]; disponible: boolean }> => {
  try {
    ...
    return { disponible: true, productos: SELECCION_INICIAL.map((borrador) => {
      ...
      return { ...borrador, ...fila, slug: fila.slug || borrador.slug, precio_mayoreo: Number(fila.precio_mayoreo) > 0 ? Number(fila.precio_mayoreo) : null };
    }) };
  } catch {
