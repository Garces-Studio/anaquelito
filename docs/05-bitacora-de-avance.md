...
## 2026-07-23 (carrito premium sin modales, navegación con efecto y panel por pestañas)

- **Cajón lateral del carrito (`src/componentes/carrito/CajonCarrito.tsx`)**: nuevo panel deslizante desde la derecha, montado global en `layout.tsx`. Muestra el pedido completo con miniatura de cada dulce, control de cantidad −/+, eliminar, subtotal en vivo y botones "Ir a pagar" / "Ver carrito completo". Cierra con Escape, clic en el fondo o la X, y bloquea el scroll de fondo mientras está abierto. Sustituye la necesidad de navegar a `/carrito` para revisar el pedido — venta sin fricción, sin modales que tapen todo.
- **Avisos flotantes al agregar**: `ContextoCarrito` ahora emite un aviso (foto del dulce + palomita + botón "Ver") cada vez que se agrega un producto; se autodescarta a los 3.2s. Reemplaza el viejo texto "✓ Agregado" dentro del botón.
- **Botón "Agregar" inteligente (`BotonAgregar.tsx`, unificado)**: si el dulce ya está en el pedido, la tarjeta muestra un control "− N en pedido +" en lugar de volver a decir "Agregar". Un solo componente ahora sirve al catálogo y al detalle (se eliminó el `BotonAgregarCatalogo` duplicado). El artículo del carrito ahora guarda también su `imagen` para pintarla en el cajón y en `/carrito`.
- **Envío gratis con barra de progreso**: nueva constante `ENVIO_GRATIS_DESDE` ($1,500, provisional) en `ContextoCarrito`. El cajón y la página `/carrito` muestran cuánto falta y una barra que se pone verde al desbloquearlo. Palanca de venta para subir el ticket promedio.
